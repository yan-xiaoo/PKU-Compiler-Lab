...
impl IrGen {
    pub(crate) fn generate_add_statement(&self, function_data: & mut FunctionData, block: &BasicBlock, expr: &function_ast::AddExp) -> Value {
        match expr {
    ...
    pub(crate) fn generate_mul_statement(&self, function_data: & mut FunctionData, block: &BasicBlock, expr: &MulExp) -> Value {
        match expr {
